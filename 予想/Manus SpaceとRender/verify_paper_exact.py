...
- 除外: SG・G1などの特殊レース（grade != '一般' を除外）

論文の検証期間:
- 分析期間: 2018年3月〜5月 → 回収率122%
- 検証期間1: 2017年3月〜5月 → 回収率101.8%
- 検証期間2: 2018年10月〜12月 → 回収率100.2%
"""
...
def verify_paper_conditions():
    ...
    for period_name, start_date, end_date in periods:
        ...
        # ステップ1: 大村競艇場の対象レースを取得（当地成績6.5以上、一般戦のみ）
        # historical_programsから1号艇の当地勝率を取得
        query = """
        WITH target_races AS (
            SELECT DISTINCT
                ...
                hp.stadium_code,
                hp.race_no,
                hp.local_win_rate as boat1_local_win_rate
            FROM historical_programs hp
            WHERE hp.stadium_code = '24'  -- 大村競艇場
              AND hp.race_date BETWEEN %s AND %s
              AND hp.boat_no = '1'  -- 1号艇
              AND hp.local_win_rate >= 6.5  -- 当地成績6.5以上
        )
        SELECT 
            tr.race_date,
            tr.stadium_code,
            tr.race_no,
            tr.boat1_local_win_rate
        FROM target_races tr
        ORDER BY tr.race_date, tr.race_no
        """
        ...
        # ステップ2: 各レースの2連単1-3と2連複1-3の払戻金を取得
        total_investment = 0
        total_payout = 0
        hit_count = 0
        ...
        for race_date, stadium_code, race_no, local_win_rate in target_races:
            ...
            cur.execute("""
                ...
                WHERE race_date = %s AND stadium_code = %s AND race_no = %s
                  AND bet_type = '2連単' AND combination = '1-3'
            """, (race_date, stadium_code, race_no))
            nitan_result = cur.fetchone()
            nitan_payout = nitan_result[0] if nitan_result else 0
            
            # 2連複1-3の払戻金を取得
            cur.execute("""
                SELECT payout FROM historical_payoffs
                WHERE race_date = %s AND stadium_code = %s AND race_no = %s
                  AND bet_type = '2連複' AND combination = '1-3'
            """, (race_date, stadium_code, race_no))
            nifuku_result = cur.fetchone()
            nifuku_payout = nifuku_result[0] if nifuku_result else 0
            
            # オッズの高い方を選択（払戻金 = オッズ × 100円）
            # 払戻金が高い方 = オッズが高い方
            selected_payout = max(nitan_payout, nifuku_payout)
            
            total_investment += 100  # 1レースあたり100円
    ...
    print(f"\n{'='*60}")
    print("追加検証: 当地成績の範囲別回収率（論文の図2を再現）")
    print(f"{'='*60}")
    ...
    # 結果をJSONファイルに保存
    with open('/home/ubuntu/paper_exact_verification.json', 'w', encoding='utf-8') as f:
        json.dump(results, f, ensure_ascii=False, indent=2, default=decimal_to_float)
    
    print(f"\n結果を /home/ubuntu/paper_exact_verification.json に保存しました")
