            ...
            if current_stadium and current_race_no:
                ...
                if result_match:
                    ...
                    try:
                        ...
                        # 展示タイムが ". ." などの場合のログは多すぎるので抑制するか、必要なら出す
                        # logger.warning(f"Exhibition time parse error: '{exhibition_time_str}' in line: {stripped}")
                        pass
                    ...
                    # 着順行っぽいのにマッチしなかった場合のデバッグログ
                    # 数字で始まっている、または F/L/K... で始まっている場合
                    if re.match(r'^\s*(\d{2}|[FKLSE失転落沈妨欠])\s+\d', stripped):
                        logger.warning(f"Unmatched result line: {stripped}")
...
def parse_payoffs_from_result_file(filepath):
    ...
    try:
        ...
        for line in lines:
            ...
            # 複数行にわたる場合あり（最大3組）
            # 継続行は「数字-数字」で始まる（strippedは既にstrip()済みなので先頭スペースは消えている）
            elif stripped.startswith('拡連複') or (in_payoff_section and re.match(r'^\d-\d\s+\d+\s+人気', stripped)):
...
def import_programs_to_db(year_month):
    ...
    for filename in os.listdir(extracted_dir):
        ...
        all_programs.extend(parse_program_file(filepath))

    return save_programs_to_db(all_programs)


def save_programs_to_db(programs, database_url=None):
    """番組表データをDBに保存"""
    if not programs:
        return 0
                ...
                for r in results:
                    try:
                        ...
                        cur.execute('''
                            ...
                            (race_date, stadium_code, race_no, boat_no, racer_no, rank, race_time, exhibition_time)
                        ...
                        ''', (
                            ...
                            r['boat_no'], r['racer_no'], r['rank'], r['race_time'], r.get('exhibition_time')
                        ))
                        total_count += 1
                    except Exception as e:
                        continue
...
def run_yesterday_import():
    """
    前日の結果をLZHファイルから取得してデータベースに保存
    毎日9:00 JSTに実行される
    """
    from lzh_result_batch import run_morning_batch
    logger.info("=== 前日結果インポート開始（LZHバッチ） ===")
    result = run_morning_batch(DATABASE_URL)
    if result:
        logger.info(f"前日結果インポート完了: {result['saved_count']}件")
    else:
        logger.warning("前日結果インポートに失敗しました（LZHファイルが未公開の可能性）")
    return result


def run_date_range_import(start_date: str, end_date: str):
    """
    日付範囲を指定してLZHファイルから結果を取得

    Args:
        start_date: 開始日 (YYYYMMDD形式)
        end_date: 終了日 (YYYYMMDD形式)
    """
    from lzh_result_batch import run_date_range_batch
    logger.info(f"=== 日付範囲インポート開始: {start_date} 〜 {end_date} ===")
    results = run_date_range_batch(start_date, end_date, DATABASE_URL)

    # サマリー表示
    if results:
        success_count = len([r for r in results if r.get('status') == 'success'])
        total_saved = sum(r.get('saved_count', 0) for r in results)
        logger.info(f"=== 日付範囲インポート完了: {success_count}/{len(results)}日成功、{total_saved}件保存 ===")
    else:
        logger.warning("日付範囲インポートに失敗しました")

    # 番組表データもバックフィル
    logger.info(f"=== 番組表データ取得開始（範囲指定） ===")
    ...
    while current_date <= end_dt:
        ...
        # BファイルのURL
        url = f"{PROGRAM_BASE_URL}/{ym}/b{yymm}{dd}.lzh"
        filepath = os.path.join(PROGRAM_DOWNLOAD_DIR, ym, f"b{yymm}{dd}.lzh")

        logger.info(f"番組表ダウンロード: {target_date_str}")

        # ダウンロード
        downloaded = download_file(url, filepath)
        if downloaded:
            output_dir = os.path.join(PROGRAM_EXTRACTED_DIR, ym)
            extract_lzh(downloaded, output_dir)

            # パースして保存
            base_fn = f"B{yymm}{dd}.TXT"
            txt_path = os.path.join(output_dir, base_fn)
            if not os.path.exists(txt_path):
                 txt_path = os.path.join(output_dir, base_fn.lower())

            if os.path.exists(txt_path):
                programs = parse_program_file(txt_path)
                count = save_programs_to_db(programs)
                logger.info(f"{target_date_str} 番組表: {count}件保存")
            else:
                logger.warning(f"番組表ファイルが見つかりません: {base_fn}")
...
def run_backfill_import():
    ...
    3. その間の日付だけをダウンロード
    """
    from lzh_result_batch import run_backfill_batch
    logger.info("=== バックフィルインポート開始 ===")
    results = run_backfill_batch(DATABASE_URL)

    # サマリー表示
    if results:
        success_count = len([r for r in results if r.get('status') == 'success'])
        total_saved = sum(r.get('saved_count', 0) for r in results)
        logger.info(f"=== バックフィルインポート完了: {success_count}/{len(results)}日成功、{total_saved}件保存 ===")
    else:
        logger.info("バックフィル不要（欠損日なし）")
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        ...
        print("  python import_historical_data.py yesterday      - 前日の結果をLZHからインポート")
        print("  python import_historical_data.py range YYYYMMDD YYYYMMDD - 日付範囲を指定してインポート")
        print("  python import_historical_data.py backfill        - 欠損分を自動取得（LZHファイルチェック）")
        sys.exit(1)
    ...
    if command == 'download':
        ...
        run_yesterday_import()
    elif command == 'range':
        if len(sys.argv) < 4:
            print("エラー: 日付範囲を指定してください")
            print("例: python import_historical_data.py range 20260117 20260123")
            sys.exit(1)
        start_date = sys.argv[2]
        end_date = sys.argv[3]
        run_date_range_import(start_date, end_date)
